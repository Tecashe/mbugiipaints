...
interface Order {
  ...
  total: number
  createdAt: string
  orderItems: Array<{ artwork: { title: string } }>
}
...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    setIsLoading(true)
    try {
      const [artworksRes, classesRes, ordersRes, inquiriesRes] = await Promise.all([
        fetch('/api/artworks'),
        fetch('/api/classes'),
        fetch('/api/orders'),
        fetch('/api/inquiries'),
      ...
      // Calculate stats
      const availableArtworks = artworksData.filter((a: Artwork) => a.status === 'AVAILABLE').length
      const activeClasses = classesData.filter((c: Class) => c.status === 'ACTIVE').length
      const totalStudents = classesData.reduce((sum: number, c: Class) => sum + c._count.enrollments, 0)
      ...
      <div className="min-h-screen bg-background">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center space-x-4">
                ...
                <Button size="sm" className="font-light tracking-wide" asChild>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="overview" className="space-y-8 mt-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="p-6">
                  ...
                  <p className="text-sm font-light text-muted-foreground mt-2">
                    {stats.availableArtworks} available
                  </p>
                </Card>
                ...
                <Card className="p-6">
                  ...
                  <p className="text-sm font-light text-muted-foreground mt-2">{stats.totalStudents} students</p>
                </Card>

                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-light text-muted-foreground">Revenue</p>
                      <p className="text-3xl font-light text-foreground">${stats.monthlyRevenue.toLocaleString()}</p>
                    </div>
                    <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
                      <DollarSign className="w-6 h-6 text-primary" />
                  ...
                  <p className="text-sm font-light text-muted-foreground mt-2">All time</p>
                </Card>

                <Card className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-light text-muted-foreground">Inquiries</p>
                      <p className="text-3xl font-light text-foreground">{stats.totalInquiries}</p>
                    </div>
                    <div className="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
                      <Mail className="w-6 h-6 text-primary" />
                    </div>
                  </div>
                  <p className="text-sm font-light text-muted-foreground mt-2">Total messages</p>
                </Card>
              </div>
              ...
              <div className="grid lg:grid-cols-2 gap-8">
                <Card className="p-6">
                  ...
                  <div className="space-y-4">
                    {orders.slice(0, 3).map((order) => (
                      <div key={order.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                        <div className="flex-1">
                          <p className="font-light text-foreground">{order.user.name || order.user.email}</p>
                          <p className="text-sm font-light text-muted-foreground">
                            {order.orderItems[0]?.artwork.title}
                          </p>
                        ...
                        <div className="text-right">
                          <p className="font-light text-foreground">${order.total}</p>
                          <Badge className={getStatusColor(order.status)}>{order.status}</Badge>
                        </div>
                      </div>
                    ))}
                  </div>
                  <Button variant="outline" className="w-full mt-4 font-light bg-transparent" onClick={() => setActiveTab("orders")}>
                    View All Orders
                  </Button>
                </Card>
                ...
                <Card className="p-6">
                  ...
                  <Button variant="outline" className="w-full mt-4 font-light bg-transparent" onClick={() => setActiveTab("classes")}>
                    Manage Classes
                  </Button>
                </Card>
              </div>
                  ...
                  <Card key={artwork.id} className="overflow-hidden">
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        <h3 className="font-light text-foreground">{artwork.title}</h3>
                        <p className="text-sm font-light text-muted-foreground">{artwork.medium}</p>
                        <p className="text-sm font-light text-muted-foreground">{artwork.dimensions}</p>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="font-light text-primary">${artwork.price}</span>
                        <span className="text-sm font-light text-muted-foreground">
                          {artwork._count.orderItems} orders
                        </span>
                      </div>
                    </div>
                  </Card>
            ...
            {/* Other tabs remain similar - just replace mock data with real data */}
            <TabsContent value="classes" className="space-y-6 mt-8">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-light text-foreground">Art Classes</h2>
              </div>
              <div className="grid gap-6">
                {classes.map((cls) => (
                  <Card key={cls.id} className="p-6">
                    <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-4 text-sm font-light text-muted-foreground">
                          <span>Students: {cls._count.enrollments}/{cls.maxStudents}</span>
            ...
            <TabsContent value="orders" className="space-y-6 mt-8">
              <h2 className="text-2xl font-light text-foreground">Orders</h2>
              <div className="space-y-4">
                {orders.map((order) => (
                  <Card key={order.id} className="p-6">
                    <div className="flex justify-between items-start">
                      <div className="space-y-2">
                        <h3 className="font-light text-foreground">{order.user.name || 'Guest'}</h3>
                        <p className="text-sm font-light text-muted-foreground">{order.user.email}</p>
                        <p className="text-sm font-light text-muted-foreground">
                          {new Date(order.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                      <div className="text-right space-y-2">
                        <div className="text-xl font-light text-foreground">${order.total}</div>
                        <Badge className={getStatusColor(order.status)}>{order.status}</Badge>
                      </div>
                    </div>
                  </Card>
            ...
            <TabsContent value="inquiries" className="space-y-6 mt-8">
              <h2 className="text-2xl font-light text-foreground">Inquiries</h2>
              <div className="space-y-4">
                {inquiries.map((inquiry) => (
                  <Card key={inquiry.id} className="p-6">
                    ...
                    <p className="font-light text-muted-foreground">{inquiry.message}</p>
                  </Card>
                ))}
              </div>
            </TabsContent>
            ...
            <TabsContent value="settings" className="space-y-6 mt-8">
              <h2 className="text-2xl font-light text-foreground">Settings</h2>
              <Card className="p-6">
                <p className="text-muted-foreground font-light">Settings coming soon...</p>
              </Card>
            </TabsContent>
          </Tabs>
        </div>
